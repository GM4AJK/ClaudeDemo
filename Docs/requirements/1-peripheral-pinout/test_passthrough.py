...
class Scope:
    ...
    def pkpk(self, ch):
        """Return peak-to-peak voltage on channel ch (1 or 2)."""
        raw = self.query(f'C{ch}:PAVA? PKPK', delay=0.5)
        # Response: "C1:PAVA PKPK,1.23E+00V"
...
def main():
    ...
    # Clear any FFT overlay from a previous test
    sc.send(':FUNC1 OFF')

    for ch in (1, 2):
        sc.send(f'C{ch}:ATTN 10')       # probe ×10
        sc.send(f'C{ch}:CPL A1M')       # AC coupled, 1 MΩ
        sc.send(f'C{ch}:VDIV 200E-3')   # 200 mV/div
        sc.send(f'C{ch}:OFST 0')        # 0 V offset

    sc.send('TDIV 5E-3')                # 5 ms/div — ~2 cycles of 100 Hz visible
    sc.send('TRMD AUTO')                # auto trigger
